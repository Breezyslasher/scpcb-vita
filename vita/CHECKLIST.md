...
| --- | --- | --- |
| Map generation (CreateMap) | done | PC-seed compatible: the generator is now a 1:1 transcription of CreateMap (hallway walk, count pass with its grid mutation, both force passes, SetRoom queues incl. the live Rnd(0.2,0.6), the creation loop with CreateRoom's cumulative-commonness pick, the off-grid room fills, and the grid-door pass with its Rand(-3,1) open rolls) driven by Blitz3D's exact Park-Miller RNG (verified against the published bbmath.cpp) and the source's GenerateSeedNumber string hash - and it replays every RNG draw FillRoom makes per created room (items, doors, decals, room lights from the RMesh light entities via generated room_gen_data.h, and the 860-1 forest's GenForestGrid/PlaceForest heightmap pass), because those draws interleave with the template picks. The same seed string should therefore produce the PC game's map, with the source's own caveats pinned: KEY2_SPAWNRATE is fixed to 5 and I_005 ChanceToSpawn to 1 (the PC rolls these from the wall clock BEFORE seeding, so even two PC runs of one seed can differ in filler rooms past the White Key room), and S-NAV/E-Reader achievement unlocks are assumed locked. Float32 arithmetic is pinned with -ffp-contract=off. Needs a PC-side comparison to certify; grid doors now come from the same stream (open states included) |
