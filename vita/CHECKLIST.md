...
| SCP-106 | mostly done | Roaming hunter with facility BFS pathfinding + phase-behind; femur-breaker recontainment gated on the magnet lever; corrosion pool on spawn and a footfall trail; corrodes the sliding/heavy doors it passes (texture swap); a trailing faded second body wells up behind it at distance; the lens zooms inward when it holds your gaze (dread pulse, also at the throne); grab-and-wrench attack on contact (frozen input, view twist) before the pocket-dimension drag; spawn timer counts down twice as fast on the aggressive difficulties. Remaining: room-DisableDecals spawn gating (the port doesn't parse that room field), a true post-process blur (approximated by the zoom), and the Tesla/HID retreat state (no Tesla gates / Micro-HID in the port) |
