...
| Elevators | done | Riding an elevator closes the doors, blacks out for the ride (Moving loop + Close/Open sounds), then arrives at the destination and opens. Real destinations: the gate_a_entrance / gate_b_entrance elevators now travel to the actual gate_a / gate_b surface rooms (appended off-grid, each with a return elevator) and back - so the gate cars go somewhere real. The room2_mt maintenance cars now ride down to a maintenance-tunnel area too - a self-contained tunnel cross (mt4.rmesh) overlaid off-grid like the intro cell, with a return elevator - and back. Any remaining elevator fast-travels to the next elevator in the map. Input is frozen for the ride. (The source's full 19x19 procedural tunnel grid is reduced to the single arrival tunnel; the deeper maze/generator tiles aren't laid out.) |
