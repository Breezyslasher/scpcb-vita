...
| SCP-682 | done | The e_682_roar set-piece in room2_5_hcz/room3_hcz/room2_5_ez: a long countdown with a distant tremor that builds, the Roar, and at the climax its huge arm (scp_682_arm.b3d) smashes through and sweeps the room - a glancing blow shoves and hurts the player - then retracts. Faithful to the source's role: 682 is a contained roarer / scripted scare, never a roaming enemy (its only other appearance is the Gate B ending) |
...
| SCP-1499-1 | done | Donning the SCP-1499 mask (use the item) exiles the player to dimension_1499 - an off-grid room appended like the pocket dimension - where the hooded people (scp_1499_1.b3d) ring the arrival point, converge, and kill on contact while sanity bleeds. Taking the mask off (use it again) tears you back to the facility; facility SCPs are suspended meanwhile. Streamlined vs source: the full church/king/guard set-piece and the 1499-1 citizen/guard social AI are reduced to a swarm-and-kill loop |
| SCP-860-1 | missing (needs the forest dimension) | Lives only in the SCP-860 forest dimension (procedural trees, the peek-from-behind-trees stalker); the forest dimension is not ported, so it has nowhere to spawn. Its model loads |
