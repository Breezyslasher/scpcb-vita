...
| Lightmap blend model | partial | Reverted to additive after hardware bisection: the modulate2x port of the source's 2 * diffuse * (ambient + lightmap) (Blitz TextureBlend 5) darkened the world to near-black on device - the RMESH vertex colours were applied in both passes (vc squared) and the dropped AmbientLightRoomTex floor let dim lightmaps multiply to black. A corrected modulate (vertex colours applied once, small ambient floor added to the lightmap term) is the faithful target but needs on-device iteration; additive is the shippable state |
