...
| SCP-914 refining | partial | The refinement machine in cont1_914 works: the knob (scp_914_knob.b3d) and control key render, interacting at the knob cycles the five settings (Rough / Coarse / 1:1 / Fine / Very Fine), and interacting at the input booth runs the currently-selected inventory item through - the machine whirs (914Refine.ogg), and after a few seconds the output drops in the output booth. The refine table (Use914) is partially ported: keycards step down/up by setting (Rough shreds, Coarse a level down, 1:1 the famous Playing Card, Fine a level up, Very Fine two up, past L5 a Mastercard); every other item is destroyed on Rough/Coarse and passed through otherwise. Not yet ported: the full per-item transformation table (gas mask -> hazmat, NVG -> SCRAMBLE, SCP-148, severed hands, first-aid, etc.) and the booth-door open/close animation (approximated by the timer) |
...
| SCP-205 | done | The shadow lamps in cont1_205 (e_cont1_205): a skinned shadow demon (scp_205_demon.b3d) rises out of the chamber floor and looms while the player is in the room, moaning (Horror.ogg) and gnawing sanity. It is seen chiefly on the observation monitor - the cont1_205 security camera's feed aims at the demon (not the player) so it fills the screen, using the live monitor feed. Approximated: the source's three-demon + rising-woman set-piece and the lethal player-grab are compressed into one rising demon and a dread swell (no instant kill) |
| SCP-372 | done | The peripheral jumper in cont3_372 (UpdateNPCType372): a skinned creature (scp_372.b3d) that stays hidden, then now and then flits to a spot ~450 raw off to your side and lingers, bobbing and spinning - but the instant you centre it in your gaze (within ~22 deg of the view forward) it darts back toward the edge of sight, with the occasional rustle. Purely a startle; it never touches you. It vanishes after a short while and re-appears elsewhere. Approximated: the exact blink-window timing of the source reveal |
