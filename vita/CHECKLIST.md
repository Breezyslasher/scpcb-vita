...
| Lightmap blend model | partial | Additive, now composited in a SINGLE pass: the lightmap rides texture unit 1 with env GL_ADD instead of a second additive draw per batch - the old two-pass scheme doubled both the draw-call count and the fill, the two measured fps walls. (The earlier modulate2x attempt stays off: hardware bisection showed it multiplied dim rooms to black - vertex colours applied twice with no ambient floor.) The lightmap now adds untinted (previously x vertex colour), marginally closer to the source's un-tinted lightmap term |
...
| SCP-939 | done | A blind pack predator prowling room3_storage that hunts by the noise the player makes (playerNoise: loud sprinting draws it from across the room, a still crouch slips past). Patrol -> alert (stalks the sound) -> attack (charges and bites, each bite injuring; enough bites, injuries > 4, kill "SCP-939"); attack/horror voice lines. It patrols - drifting between wander points around its storage room - and mimics human voices to draw the curious closer; not ported: the exact source waypoint graph |
...
| Per-room events (Events_Core, ~100 events) | mostly done | Beyond the original 3 (173 ambush, "trick" scare, 682 roar), ~28 more FillRoom-scripted events now run through a two-slot-per-room system with skinned Class-D/guard cameo figures and world props: the LCZ dead-end janitor abduction (106 pulls him under, corrosion pool, may then rise after you), the EZ shelter guard sealing the upper door, the closets double-kill (two staff, 173 teleports to each neck-snap, wallet drop), the LCZ office Gonzales corpse, the cycling ceiling fan, the blink-teleporting PA duck + the saxophone duck + the healing medibay duck + the squeakable storage rubber duck, the 1048 doodle scampers (room3_hcz + test_hcz), Document SCP-789-J with its fanfare, the suicide guard, the medibay 008-zombie ambush, 049 stalking room2_sl with his voice line, the shaft/gw_2/room3_2_hcz/room4_2_hcz corpses (gw_2 with alarm), the room3_storage 939 victims, the servers_hcz 096 scene (doors lock, guard dies when 096 triggers, bloody Level 3 card drops), Emily's scream behind the corroding cont2_500_1499 door, the 409 victim + crystal decal + Level 5 card + document, the 008 chamber infection (glass cut, creeping bloodloss death), the 1162-ARC hole (random item trade, or flesh when empty-handed, with its death report), Dr. L's office dread zone, decontamination-gas flood in room2_6_hcz, the 106 hall emergences (2/5_hcz), the ceiling victim drop and the corrosion sinkhole into the pocket dimension, the door-closing scare, and the 035 victim in room2_ez. Also ported: the SCP-970 loop corridor (room2_storage wraps at the seam with per-loop escalation - stolen items, blood, strange notes, the Benjamin corpse pose, Franklin3 on the radio, the vanishing guard), the broken guard elevator (he walks the walk cycle in, the doors shut, ElevatorDeath plays and the shakes hit on the source's beats), SCP-294 (two quarters vend a Cup with the dispense sounds; the drink-name GUI and per-drink effects are not ported) + SCP-458's endless pizza, SCP-1123's flashback (touching the skull brings the officer, his three lines, the gunshot - staged around the player rather than in the source's hidden replica cell, whose separate cont2_1123_cell.rmesh the port does not load), SCP-066 rolling about humming Notes/Eric/Beethoven (prop, not the full NPC), the gateway airlock cycle, and checkpoint mirroring (the room2_sl generator lever is the facility power: checkpoint gates lock and their monitors go dead until it is thrown; maps without room2_sl stay powered). Guards now use their walk cycle (frames 1-38) while moving. The gate entrance scripts now run too: the big surface-gate doors answer only to the electrical centre's remote-door lever (room2c_ec, whose lights also cut out with Horror7/Blackout and whose lever set is live; maps without the room leave the gates unlocked), and SCP-106 laughs on the way up Gate A while uncontained. SCP-294 pours from a menu of its classics with each drink's message, sound and effect (coffee restores stamina, alcohol blurs, sometimes the cup is just empty) - the typed-name GUI itself is not portable to the Vita. Loose items now ride the SCP-970 loop across the seam with the player |
